...
abstract class REST_Controller extends CI_Controller {
    ...
    public function _remap($object_called, $arguments = [])
    {
        ...
        if ($this->config->item('rest_enable_keys') && $use_key && $this->_allow === FALSE)
        {
            ...
            $this->response([
                "status" => FALSE,
                "message" => sprintf($this->lang->line('text_rest_invalid_api_key'), $this->rest->key)
            ], self::HTTP_OK);
    ...
    protected function  _detect_permision($usuario){
        
        $resp = false;        
        $class = $this->router->fetch_class();
        $method = $this->router->fetch_method();
        $method_type = $this->input->method();
        //echo $usuario->privilegio."**".$class  ."===departamentoServices**".$method ."=== listar" .$method_type ."=== POST";
        //echo ($class === "departamentoServices" && $method === "listar" && $method_type === "get");
        if($usuario){
            switch ($usuario->privilegio) {
                case 1://SERVICIOS HABILITADOS PARA EL PUBLICO
                    $resp = ($class === "user" && $method === "index" && $method_type === "post") ||
                    $resp = ($class === "departamentoServices" && $method === "listar" && $method_type === "get") ||
    ...
    protected function _detect_api_key()
    {
        ...
        {   
            ...
            if (!($row = $this->rest->db->select("*")->from($this->config->item('rest_keys_table')." k")->join("usuario u","u.id=k.usuario")->where($this->config->item('rest_key_column'), $key)->get()->row()))
            {
                  return FALSE;                
            }
			//$this->key_row = $row;
			//$usuario = $this->rest->db->where("id", $row->usuario)->get("usuario")->row();
			$usuario = $row;
			unset($usuario->password);
			unset($usuario->user_id);
			unset($usuario->level);
			unset($usuario->ignore_limits);
			unset($usuario->is_private_key);
			unset($usuario->ip_addresses);
			unset($usuario->ciudades);
			unset($usuario->latitud);
			unset($usuario->longitud);
			$this->user = $usuario;
			
			if(!$this->_detect_permision($usuario)){
				return FALSE;
			}
        ...
        if($this->router->fetch_class() === "user" && $this->router->fetch_method() === "login" && $this->input->method() === "post")
            return TRUE;
